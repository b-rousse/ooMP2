...
namespace libint2 {
  ...
  class BasisSet : public std::vector<libint2::Shell> {
    public:
       ...
       * @param throw_if_no_match If true, and the basis is not found for this atomic number, throw a std::runtime_error.
       *       Otherwise omit the basis quietly.
       * @throw std::runtime_error if throw_if_no_match is true and for at least one atom no matching basis is found.
       ...
       *         <li> hardwired to directory \c NONE/share/libint/2.4.2/basis </li>
       *       </ol>
       */
      BasisSet(std::string name,
        ...
        for(auto a=0; a<atoms.size(); ++a) {
          ...
          for(auto comp_idx=0; comp_idx!=component_basis_sets.size(); ++comp_idx) {
            ...
            if (!component_basis_set[Z].empty()) {  // found? add shells in order
      ...
      /// @param throw_if_no_match If true, and no atom matches the origin of a shell, throw a std::runtime_error.
      ///        Otherwise such shells will be mapped to -1.
      /// @return the map from shell index to the atom in the list \c atoms that coincides with its origin;
      /// @throw std::runtime_error if throw_if_no_match is true and for at least one shell no matching atom is found.
      static std::vector<long> shell2atom(const std::vector<Shell>& shells, const std::vector<Atom>& atoms, bool throw_if_no_match = false) {
        ...
        for(const auto& s: shells) {
          ...
          if (throw_if_no_match && !found_match)
            throw std::runtime_error("shell2atom: no matching atom found");
...
#else
          path = std::string("NONE/share/libint/2.4.2") + std::string("/basis");
#endif
        }
        // validate path
        bool error = true;
